...
class Geometry extends THREE.EventDispatcher {
	...
	constructor(radius, widthSegments, heightSegments) {
		...
		function interpolateVertex(u, v, t) {
			const maxLng = Math.PI * 2;
			const maxLat = Math.PI;

			const sphereX = - this.radius * Math.cos(u * maxLng) * Math.sin(v * maxLat);
			const sphereY = - this.radius * Math.cos(v * maxLat);
			const sphereZ = this.radius * Math.sin(u * maxLng) * Math.sin(v * maxLat);

			const planeX = u * this.radius * 2 - this.radius;
			const planeY = v * this.radius * 2 - this.radius;
			const planeZ = 0;

			const x1 = interplolate(sphereX, planeX, t);
			const y1 = interplolate(sphereY, planeY, t);
			const z = interplolate(sphereZ, planeZ, t);
		...
		function interpolateUV(u, v, t) {
			const lat = (v - 0.5) * 89.99 * 2 / 180 * Math.PI; // turn from 0-1 into lat in radians
			const sin = Math.sin(lat);
			const normalizedV = 0.5 + 0.25 * Math.log((1 + sin) / (1 - sin)) / Math.PI;
		...
		for (y = 0; y < this.heightSegments; y++) {
			...
			for (x = 0; x < this.widthSegments; x++) {
				...
				const n2 = this.vertices[v2].clone().normalize();
				const n3 = this.vertices[v3].clone().normalize();
				const n4 = this.vertices[v4].clone().normalize();

				const uv1 = uvs[y][x + 1];
				const uv2 = uvs[y][x];
				const uv3= uvs[y + 1][x];
				const uv4 = uvs[y + 1][x + 1];
	...
	toBufferGeometry() {
		...
		const poisitionAttribute = new THREE.BufferAttribute( positions, 3 );

		buffergeometry.setAttribute( 'position', this.copyVector3sArray( poisitionAttribute, geometry.vertices ) );
		...
		if ( geometry.normals.length > 0 ) {
			...
			buffergeometry.setAttribute( 'normal', this.copyVector3sArray( normalAttribute, geometry.normals ) );
		...
		if ( geometry.colors.length > 0 ) {
			...
			buffergeometry.setAttribute( 'color', this.copyColorsArray( colorAttribute, geometry.colors ) );
		...
		if ( geometry.uvs.length > 0 ) {
			...
			buffergeometry.setAttribute( 'uv', this.copyVector2sArray( uvAttribute, geometry.uvs ) );
		...
		if ( geometry.uvs2.length > 0 ) {
			...
			const uv2Attribute = new THREE.BufferAttribute( uvs2, 2 );

			buffergeometry.setAttribute( 'uv2', this.copyVector2sArray( uv2Attribute, geometry.uvs2 ) );
		...
		for ( const name in geometry.morphTargets ) {
			...
			for ( let i = 0, l = morphTargets.length; i < l; i ++ ) {
				...
				array.push( this.copyVector3sArray( attribute, morphTarget.data ) );
		...
		if ( geometry.skinIndices.length > 0 ) {
			...
			buffergeometry.setAttribute( 'skinIndex', this.copyVector4sArray( skinIndices, geometry.skinIndices ) );
		...
		if ( geometry.skinWeights.length > 0 ) {
			...
			buffergeometry.setAttribute( 'skinWeight', this.copyVector4sArray( skinWeights, geometry.skinWeights ) );
	...
	copyColorsArray( bufferAttribute, colors ) {
	...
	copyVector2sArray( bufferAttribute, vectors ) {
	...
	copyVector3sArray( bufferAttribute, vectors ) {
	...
	copyVector4sArray( bufferAttribute, vectors ) {
	...
	static createBufferGeometryFromObject( object ) {
		...
		if ( object.isPoints || object.isLine ) {
			...
			buffergeometry.setAttribute( 'position', this.copyVector3sArray( positions, geometry.vertices ) );
			buffergeometry.setAttribute( 'color', this.copyColorsArray( colors, geometry.colors ) );
